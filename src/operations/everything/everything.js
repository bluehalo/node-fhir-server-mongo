 ...
 */
module.exports.everything = async (args, user, scope, path,
                                   protocol, host, resource_name,
                                   collection_name) => {
    ...
    try {
        ...
        if (collection_name === 'Practitioner') {
            return await graph(args, user, scope, practitionerEverythingGraph, path, protocol, host, resource_name, collection_name);
        } else if (collection_name === 'Organization') {
            return await graph(args, user, scope, organizationEverythingGraph, path, protocol, host, resource_name, collection_name);
        } else if (collection_name === 'Slot') {
            return await graph(args, user, scope, slotEverythingGraph, path, host, protocol, resource_name, collection_name);
        } else {
